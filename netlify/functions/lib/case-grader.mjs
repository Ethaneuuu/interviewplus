...
function lbo(x) {
  ...
  const debt = senior + junior + revolver; const exitEv = ebitda * x.exit_multiple; const exitEquity = exitEv - debt + cash; const waterfall = (equity) => Math.max(0, equity - sponsorEquity * (x.management_hurdle || 0)) * (x.management_pool || 0); const managementProceeds = waterfall(exitEquity); const sponsorProceeds = exitEquity - managementProceeds; const mom = sponsorProceeds / sponsorEquity; const lowEquity = (x.exit_multiple - .5) * ebitda - debt + cash;
  return { ...output, exit_ev: round(exitEv), exit_equity: round(exitEquity), exit_debt: round(debt), exit_cash: round(cash), mom: round(mom, 4), irr: round(mom ** .2 - 1, 6), sensitivity_low: round((lowEquity - waterfall(lowEquity)) / sponsorEquity, 4), sensitivity_high: round((((x.exit_multiple + .5) * ebitda - debt + cash) - waterfall((x.exit_multiple + .5) * ebitda - debt + cash)) / sponsorEquity, 4), management_proceeds: round(managementProceeds) };
}
